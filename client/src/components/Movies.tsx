...
const Movies = ({setInfo, setId}: CarouselProps) => {
...
dispatch(getMovies())
  ...
  return (
    <div className="mb-4">
      ...
      <h1 className="text-2xl font-bold ml-6">Movies</h1>
      </div>

      {
        movies?.length !== 0 ? <div className="grid lg:grid-cols-5 md:grid-cols-4 sm:grid-cols-3 xxs:grid-cols-2 sm:gap-4 gap-2">
      ...
      <div className="relative">
           <div className="flex flex-row dark:text-slate-400 text-sm sm:gap-4 gap-2 pt-1 items-center mx-4">
                            <h3>⭐&nbsp;{item.rate}<span className="dark:text-slate-200 text-gray-600">/5</span></h3>
                            <button
          ...
          <button className="px-3 py-2 hover:bg-black rounded-full hover:bg-opacity-50" onClick={()=>handleInfo(item._id)}>
