...
import { editRolebyId, getRole } from "../redux/thunk/adminThunk";
...
const AdminEditRole = ({ setEditRole, editRole }: prop) => {
  ...
  const { loading, role } = useSelector((state: RootState) => state.admin);
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      // Assuming `formData` and `editRole` are available in your code
      const res = await dispatch(editRolebyId({ formData, id: editRole }));

      if (editRolebyId.fulfilled.match(res)) {
  ...
  useEffect(() => {
    if (editRole) {
      dispatch(getRole(editRole)); // Fetch role data when editRole changes
    }
  }, [dispatch, editRole]);

  // Set the default values for the form once the role data is fetched
  useEffect(() => {
    if (role) {
      reset({
        name: role.name, // Pre-fill the role name
      });
      setPreview(role.url); // Pre-fill the image preview if available
    }
  }, [role, reset]);
